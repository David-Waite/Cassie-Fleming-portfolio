  ...
  for (let i = 0; i < personalWorks.length; i++) {
    personalWorksElementsv2 += (
      <div>
        ...
        />
        <BookPreview
          key={personalWorks[i + 1].sys.id}
          width={
            personalWorks[i + 1].fields.image.fields.file.details.image.width
          }
          height={
            personalWorks[i + 1].fields.image.fields.file.details.image.height
          }
          src={personalWorks[i + 1].fields.image.fields.file.url}
          alt={personalWorks[i + 1].fields.title}
          slug={`/personalWorks/${personalWorks[i + 1].fields.slug}`}
        />
      </div>
    );
    i++;
